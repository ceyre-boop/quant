...
TRADING APPLICATION:
  ...
  Sentiment diffusion:
    ...
    (Relevant for meme stock dynamics, crypto cycles)
```
...
> Subjective analysis says "it looks like it works." Statistics says "here is the probability this is noise."

### Sample Size Requirements

```
RULE: You need sufficient INDEPENDENT observations. Not calendar time.

For strategy validation:
  Minimum: 100 trades (can detect edge only if it's large)
  Good: 300 trades (can detect moderate edge)
  Strong: 600+ trades (can detect small but real edge)

INDEPENDENCE CHECK:
  IF trades overlap in time (e.g., 30-day hold, entering weekly) → they are NOT independent
  Number of independent observations ≈ Total period / Average hold time
  20 years × 52 weeks / 3-week average hold = ~347 independent observations

AVOIDING THE LOOK-AHEAD TRAP:
  Features must be available AT THE TIME of the trade
  Use point-in-time data (many data providers backfill; this destroys backtests)
  Earnings revisions, index membership, financial restatements all affect this
```

### The Multiple Testing Problem (The Silent Killer of Backtests)

```
PROBLEM:
  IF you test 100 strategies on the same data
  And use p < 0.05 as threshold for significance
  Then 5 strategies will appear significant BY CHANCE (5% × 100 = 5)
  You will select those 5 and think you found alpha
  Out of sample: they fail

SOLUTIONS:

1. Bonferroni Correction (conservative):
   New threshold = 0.05 / N_tests
   If you tested 50 strategies → threshold = 0.001
   Only strategies with p < 0.001 are truly significant

2. Benjamini-Hochberg (FDR Control):
   Controls False Discovery Rate rather than per-test error
   Less conservative than Bonferroni; preferred for larger test sets
   Sort p-values; compare to (i/N) × 0.05 threshold

3. Holdout Set (Simplest and most practical):
   Never touch the last 30% of your data during development
   When you have ONE final strategy → test on holdout
   If it works there → you have genuine evidence
   If it fails → the strategy was overfit

4. Walk-Forward Validation:
   Train on 3 years → test on 1 year → roll forward → repeat
   Average OOS performance = honest estimate of live performance
   Degradation from in-sample to OOS = degree of overfitting
```

### The T-Statistic of Sharpe Ratio

```
KEY QUESTION: Is a Sharpe ratio of X statistically significant given my data?
...
Significance thresholds (t > 2 for p < 0.05):
  SR = 0.5, T = 16 years → t = 2.0 → just barely significant
  SR = 1.0, T = 4 years  → t = 2.0 → significant
  SR = 2.0, T = 1 year   → t = 2.0 → significant

IMPLICATION:
  A Sharpe of 0.8 with only 2 years of data has t = 1.13 → NOT significant
  You cannot know if it's real
  A Sharpe of 0.8 with 10 years has t = 2.53 → significant
  → Long histories matter; be skeptical of short backtests

For non-Normal returns (which markets have):
  Use the deflated Sharpe ratio (López de Prado):
  Accounts for skewness, kurtosis, and multiple testing simultaneously
```
...
def walk_forward_test(data, strategy_fn, train_years=3, test_years=1):
    results = []
    n = len(data)
    freq = 252  # daily
    train_size = train_years * freq
    test_size = test_years * freq
    ...
    for start in range(0, n - train_size - test_size, test_size):
        ...
        # Fit model on train; get parameters
        params = strategy_fn.fit(train)

        # Evaluate strictly on test (never seen by model)
        ...
        results.append({
            ...
            'n_trades': len(oos_returns[oos_returns != 0])
        })

    return pd.DataFrame(results)

# Minimum bar for a real strategy:
#   Average OOS Sharpe > 0.5
#   Positive return in > 60% of test windows
#   No single test window worse than -20%
```
