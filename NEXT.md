...
### 2026-07-27 · TICK-024 staged patch repaired (corrupt diff fixed, still NOT applied)
`research/TICK-024_staged_patch.diff` was corrupt (`git apply --check` → "corrupt patch at line
61": mismatched hunk-header line counts vs body — a diff-authoring bug, not drift against HEAD)
and would have failed the July-28 unlock apply step. Regenerated by making the intended edit to
`sovereign/forex/forex_backtester.py` in the working tree, capturing it with `git diff` (correct
hunk framing), then `git checkout --` to revert — the file was NEVER left modified. New diff
targets the same 3 hunks as before (import `ratediff_financing_rate` from the not-yet-written
`sovereign/forex/swap_model.py`, comment update on `SWAP_RATES_ANNUAL`, `_apply_costs` swap-rate
lookup swapped for the per-trade rate-differential model). `git apply --check` now PASSES.
TICK-044's staged patch untouched. Isolation test green (`test_pipeline_does_not_import_sovereign`
