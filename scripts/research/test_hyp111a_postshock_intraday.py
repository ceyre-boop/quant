#!/usr/bin/env python3
"""HYP-111a — THE test. Runs once. --fetch-only fills the minute cache (computes nothing);
...
def main(argv) -> int:
    ...
    if a.fetch_only:
        ...
        for sym, d in need:
            if (th.CACHE_1M / f"{sym}_{d}.parquet").exists():
                continue
            th.stock_1m(sym, d); done += 1
            if done % 50 == 0: print(f"  fetched {done}")
            if done >= a.max_days: break
        missing = sum(1 for s, d in need if not (th.CACHE_1M / f"{s}_{d}.parquet").exists())
        print(f"fetched {done}; still missing {missing} of {len(need)}"); return 0

    missing = [f"{s}_{d}" for s, d in need if not (th.CACHE_1M / f"{s}_{d}.parquet").exists()]
    if missing:
    ...
    df.to_parquet(OUT / "hyp111a_trades.parquet", index=False)
    ...
    print(f"(x) ex-2025 delta Sharpe {x:+.3f} {'PASS' if x_pass else 'FAIL'}")
    ...
    if data_abort or len(folds) != 15:
        ...
        verdict = "INCONCLUSIVE"
    print(f"\n=== VERDICT: {verdict}   confluence: {conf_line} ===\n")
    ...
    (OUT / "hyp111a_result.json").write_text(json.dumps(res, indent=2, default=float))
    prereg.adjudicate(HYP, verdict, f"{verdict}; confluence {conf_line}", {"oos_sharpe": sh, "p_value": float((boot <= 0).mean()),
                                                                             "result_file": "data/research/hyp111/hyp111a_result.json"})
    prereg.verify(HYP)
    return 0
