...
HYP_ID = "HYP-111"
PROBE_DATES = ["2020-03-16", "2021-12-15"]   # Alpaca loader smoke-test sessions (GLD, SPY), format only


def build_doc() -> dict:
    n_trials = prereg.mined_total() + 8          # HYP-109, regime, HYP-110, HYP-111a, HYP-112, HYP-113(x2 claims), this
    return {
        "id": HYP_ID, "slug": "postshock_intraday_retrace_continuation",
        "name": "Post-shock intraday retrace-then-continuation + next-session fade (ten ETFs, 1-min, 2020-01 → 2026-07, as scoped)",
        ...
        "provenance": ("THE scoped test. Data gate re-opened 2026-09-02: Alpaca SIP (existing key) serves full "
                       "RTH minute sessions from 2016 — verified live on 2016-03-01/2018-02-05/2020-03-16/2022-06-13 "
                       "before sealing; the repo's 2-year-wall note was wrong. No purchase. HYP-111a (2023-06+) "
                       "is a strict sub-window of this test and its result is known: the fade, not the path. "
                       "That knowledge is declared here; it shaped the SECONDARY claim, not the primary, which is "
                       "byte-for-byte the scope frozen in 0925bd9 before any minute bar."),
        ...
        "data": {
            ...
            "intraday": "Alpaca SIP 1-min, adjustment=all, RTH 09:30-15:59 ET (research/hyp111/alpaca_1m.py), cached data/cache/alpaca_1m_rth/",
            "entitlement": "Alpaca SIP historical, verified to 2016-03-01",
            "window_t1": ["2020-01-02", "2026-07-16"],
        ...
        "power_statement": ("~1,798 instrument-events on ~670 dates (scope §1). HYP-111a measured a 12.8% trigger "
                            "rate on its sub-window → expect ~230 path trades on ~200 dates for the primary: "
                            "SE ≈ 0.05%/trade, 80% power at ≈0.14%/trade — the primary is thin by construction. "
                            "The secondary (fade on ALL events) has ~670 dates: SE ≈ 0.04%/event-day, 80% power at "
                            "≈0.11%. 2020 is 22% of dates and the regime the fade has never seen."),
        ...
        "secondary_claim_fade": {
            "what": "the unconditional next-session fade: −naive_net on every instrument-event (the HYP-111a incumbent, negated)",
            "n_trials": n_trials + 1,
            "pass": ("date-block 95% CI of mean fade per event-day excludes 0 from above AND >= 7/10 instruments mean > 0 "
                     "AND 2020-only mean fade CI does not exclude 0 from BELOW (i.e. 2020 does not refute it) AND ex-2020 mean > 0"),
            "verdict_words": "FADE_HOLDS / FADE_FAILS / FADE_INCONCLUSIVE (any instrument < 30 events)",
            "descriptive": ["per-year mean and CI", "up vs down shocks", "expectancy W/avgWin/avgLoss/E", "2020-03 alone", "worst 5 trades"],
            "floor": "none — this is a conditioning claim; the per-deployed-capital denominator is recorded in HYP-113 floor_note_standing and would be sealed by a later strategy prereg",
        },
        "verdict_ladder": {
        ...
        "priors": {"operator": {"prior": "CONFIRMED (stated for the scoped hypothesis 2026-09-02)", "most_likely_failure": "data gate fails (it did on ThetaData; Alpaca re-opened it)"},
                   "claude": {"prior": "primary VALID_BUT_BELOW_FLOOR (as 111a); secondary FADE_FAILS on the 2020 component — the fade is liquidity provision and March 2020 is where it gets paid in drawdown", "most_likely_failure": "2020-03 down-shock continuation"}},
...
def main() -> int:
    ...
    a = ap.parse_args()
    if a.write:
        return prereg.write(build_doc(), "Pre-registered 2026-09-02 before any 2020-2023 event-session minute bar was read. THE scoped test (research/HYP-111_SCOPE.md) on Alpaca SIP; HYP-111a sub-window result declared known.")
    if a.verify:
