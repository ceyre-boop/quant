...
class SwingEngine:
    ...
    async def run_monthly_scan(self) -> List[SwingBias]:
        ...
        self.logger.info(
            "Monthly scan complete. Tradeable symbols: %d/%d",
            scan_log["symbols_tradeable"],
            len(symbols),
        )
    ...
    def _determine_tradeability(
        self, composite: float, layers_aligned: int, base_rate_data: Optional[Dict]
        ...
        if base_rate_data:
            ...
            if base_rate_data.get("confidence", 0) < min_confidence:
                return (
                    False,
                    f"Base rate confidence {base_rate_data['confidence']:.2f} below threshold",
                )
            if base_rate_data.get("win_rate", 0) < 0.52:
                return (
                    False,
                    f"Historical win rate {base_rate_data['win_rate']:.2f} insufficient",
                )
