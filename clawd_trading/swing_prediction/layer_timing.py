...
class TimingLayer:
    ...
    async def compute(self, symbol: str) -> TimingResult:
        ...
        signals = []
        if opex_score > 0.5:
            signals.append("post-opex")
        elif opex_score < 0:
            signals.append("opex-pressure")
        if fomc_score > 0.5:
            signals.append("post-fomc")
        elif fomc_score < 0:
            signals.append("pre-fomc")
        if quarter_score > 0:
            signals.append("fresh-quarter")
        elif quarter_score < 0:
            signals.append("rebal-pressure")
