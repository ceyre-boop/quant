...
class ReportGenerator:
    ...
    def _generate_csv(self, result: BacktestResult, filepath: Path):
        ...
        if not result.trades:
            ...
            with open(filepath, 'w') as f:
                f.write("trade_id,symbol,direction,entry_time,entry_price,exit_time,exit_price,"
    ...
    def _generate_drawdown_chart(self, result: BacktestResult, filepath: Path):
        ...
        ax.set_title(f"Drawdown Analysis - {result.config.symbol}", 
...
def print_trade_list(result: BacktestResult, limit: int = 10):
    ...
    print("=" * 100)
    print(f"{'ID':<12} {'Dir':<6} {'Entry Time':<20} {'Entry':<10} {'Exit':<10} "
          f"{'PnL':<12} {'Reason':<15}")
    print("-" * 100)
    ...
    for trade in result.trades[-limit:]:
        ...
        print(f"{trade.trade_id:<12} {trade.direction.name:<6} "
