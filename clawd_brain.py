...
def validate_prediction_against_outcomes(
    ...
    if n < 30:
        return type(
            "Result",
            (),
            {"passed": False, "p_value": 1.0, "rejection_reason": "Sample < 30"},
        )()
...
class ClawdBrain(AIBrain):  # v3.1 - Canonical
    ...
    def predict(self, data: Dict[str, pd.DataFrame]) -> List[Signal]:
        ...
        print(
            f"[ClawdBrain] Kelly stats: avg_win={self.kelly.compute_stats()[0]:.3f}, avg_loss={self.kelly.compute_stats()[1]:.3f}"
        )
