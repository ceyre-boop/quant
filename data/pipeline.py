...
class DataPipeline:
    ...
    def run_premarket(self, symbols: List[str], include_all_features: bool = True) -> Dict[str, FeatureRecord]:
        ...
        logger.info(
            f"Pre-market pipeline complete. Valid records: {sum(1 for r in results.values() if r.is_valid)}/{len(symbols)}"
        )
        return results

    def run_intraday_refresh(self, symbols: List[str]) -> Dict[str, FeatureRecord]:
