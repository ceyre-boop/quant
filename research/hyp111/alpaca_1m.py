...
CACHE_DAILY = ROOT / "data" / "cache" / "alpaca_daily"


def daily_bars(sym: str, start: str = "2016-01-04", end: str = "2026-07-16") -> pd.DataFrame:
    """Daily OHLCV, adjustment=all, cached once per symbol. Alpaca history begins 2016-01-04."""
    CACHE_DAILY.mkdir(parents=True, exist_ok=True)
    f = CACHE_DAILY / f"{sym}.parquet"
    if f.exists():
        return pd.read_parquet(f)
    rows, token = [], None
    while True:
        ...
        if token: p["page_token"] = token
        r = requests.get(f"https://data.alpaca.markets/v2/stocks/{sym}/bars", headers=_headers(), params=p, timeout=60)
        r.raise_for_status(); j = r.json(); rows += j.get("bars") or []; token = j.get("next_page_token")
        if not token: break
    df = pd.DataFrame({"date": pd.to_datetime([b["t"][:10] for b in rows]),
                       **{k: [float(b[a]) for b in rows] for k, a in (("open", "o"), ("high", "h"), ("low", "l"), ("close", "c"), ("volume", "v"))}})
    df.to_parquet(f, index=False)
    return df
