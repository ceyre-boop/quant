...
from contracts.types import (
    Direction, Magnitude, BiasOutput, RiskOutput, GameOutput,
    PositionState, RegimeState, AccountState, MarketData,
    VolRegime, TrendRegime, RiskAppetite, MomentumRegime, EventRisk,
    AdversarialRisk
)

from orchestrator.daily_lifecycle import (
    DailyLifecycle, LifecycleConfig, CyclePhase,
    create_default_lifecycle
)

from orchestrator.state_machine import (
    SymbolStateMachine, SymbolContext, SymbolState, PositionPhase,
    StateMachineManager
)
...
class TestDailyLifecycle:
    ...
    def lifecycle(self, mock_broadcaster):
        with patch('orchestrator.daily_lifecycle.FirebaseClient'):
            with patch('orchestrator.daily_lifecycle.FirebaseBroadcaster', return_value=mock_broadcaster):
    ...
    def test_run_premarket(self, lifecycle, mock_broadcaster):
        ...
        lifecycle.register_components(
            data_fetcher=lambda s: MarketData(
                symbol=s, current_price=100.0, bid=99.5, ask=100.5,
                spread=1.0, volume_24h=1000.0, atr_14=2.0,
            ...
            risk_engine=lambda s, b, f, a, m: RiskOutput(
                position_size=1.0, kelly_fraction=0.25,
                stop_price=98.0, stop_method="atr",
                tp1_price=104.0, tp2_price=106.0,
                trail_config={}, expected_value=0.5,
                ev_positive=True, size_breakdown={}
            ),
...
class TestSymbolStateMachine:
    ...
    def test_on_entry_signal_wrong_state(self, machine):
        ...
        machine.on_entry_confirmed("NAS100", PositionState(
            trade_id="T001", symbol="NAS100", direction=Direction.LONG,
            entry_price=100.0, position_size=1.0, stop_loss=98.0,
            tp1=104.0, tp2=106.0, current_price=101.0,
            unrealized_pnl=100.0, realized_pnl=0.0, status="OPEN",
            opened_at=datetime.now()
        ))  # Now in IN_POSITION
    ...
    def test_get_summary(self, machine):
        ...
        machine.on_entry_confirmed("US30", PositionState(
            trade_id="T001", symbol="US30", direction=Direction.LONG,
            entry_price=100.0, position_size=1.0, stop_loss=98.0,
            tp1=104.0, tp2=106.0, current_price=101.0,
            unrealized_pnl=100.0, realized_pnl=0.0, status="OPEN",
            opened_at=datetime.now()
