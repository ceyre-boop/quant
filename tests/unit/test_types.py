...
from contracts.types import (
    ...
    RegimeState, BiasOutput, RiskOutput, LiquidityPool, TrappedPositions,
    NashZone, GameOutput, ThreeLayerContext, FeatureRecord, FeatureSnapshot,
    EntrySignal, PositionState, AccountState, MarketData
)
...
class TestLayer3Types:
    ...
    def test_game_output_creation(self):
        pool = LiquidityPool(
            price=20000.0, strength=3, swept=False, age_bars=10,
            draw_probability=0.75, pool_type='equal_highs'
        )
        trapped = TrappedPositions(
            trapped_longs=[], trapped_shorts=[],
            total_long_pain=0, total_short_pain=0, squeeze_probability=0
        )
        game = GameOutput(
...
class TestThreeLayerContext:
    ...
    def valid_context(self):
        ...
        regime = RegimeState(
            VolRegime.NORMAL, TrendRegime.STRONG_TREND,
...
class TestExecutionTypes:
    ...
    def test_entry_signal_creation(self):
        ...
        risk = RiskOutput(
            1000, 0.15, 19500, 'atr', 20000, 20500, {}, 0.05, True, {},
            datetime.utcnow()
        )
        trapped = TrappedPositions([], [], 0, 0, 0)
        game = GameOutput(
            {'equal_highs': [], 'equal_lows': []}, None, trapped,
            0, [], 0, True, '', AdversarialRisk.LOW, datetime.utcnow()
        )
        regime = RegimeState(
            VolRegime.NORMAL, TrendRegime.STRONG_TREND,
            RiskAppetite.RISK_ON, MomentumRegime.ACCELERATING,
