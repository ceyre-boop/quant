...
def chi_squared_test(predictions: np.ndarray, actuals: np.ndarray, 
                     confidence_buckets: int = 5) -> Dict:
    """
    ...
    bucket_labels = [f"{(bucket_edges[i]*100):.0f}-{(bucket_edges[i+1]*100):.0f}%" 
